...
function Calculator() {
  ...
  const [net, setNet] = useState(-6.1);
  const handlePriceChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setPrice(event.target.value as unknown as number);
  };
  const handleCostChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setCost(event.target.value as unknown as number);
  };
  useEffect(() => {
    const fbaFee = 3.5;
    const referralFee = 2.6;
    const closingFee = 0;
    setNet(price - cost - fbaFee - referralFee - closingFee);
  }, [cost, price]);
  return (
