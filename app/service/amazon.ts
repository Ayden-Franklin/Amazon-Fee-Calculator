...
export function loadWeightRule(): Promise<string> {
  ...
  return got(url)
    .then((response) => {
      ...
      // console.log(result);
      console.log('Year, I got the page.');
      // let output = [];
      ...
      console.log(' this is the table array');
      console.log(output);
      return output;
    })
    .catch((err) => {
...
export function loadReferralTable(): Promise<string> {
  const url = 'https://sellercentral.amazon.com/gp/help/external/GTG4BAWSY39Z98Z3?language=en_US&ref=efph_GTG4BAWSY39Z98Z3_cont_6F7CN3EQS7MEGCN';
  return got(url)
    .then((response) => {
      const $ = cheerio.load(response.body);
...
export function loadClosingFee(): Promise<string> {
  const url = 'https://sellercentral.amazon.com/gp/help/external/GKD9U5REK5DKB38Y?language=en_US&ref=efph_GKD9U5REK5DKB38Y_cont_6F7CN3EQS7MEGCN';
  return got(url)
    .then((response) => {
      const $ = cheerio.load(response.body);
